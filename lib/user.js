...
import { getTimezone } from "./utils/random";
...
async function getStats() {
    try {
        const response = await axios.get(`/stats/me/`);
        return response.data;
    } catch (e) {
        prettyAxiosError(e);
    }
...
async function getPrivilegedUser() {
    try {
        const response = await axios.get(`/me/privileged/`);
        return response.data;
    } catch (e) {
        prettyAxiosError(e);
    }
...
async function updatePrivilegedSettings(form) {
    let formCopy = form;
    try {
        let data = new FormData();
        const headers = {
...
async function syncTimezone() {
    ...
    try {
        ...
        if (!isServer) {
            setCookie(null, "timezone", timezone);
        }
        await axios.post("/me/set_timezone", payload);
    } catch (e) {
        if (!isServer) {
            setCookie(null, "timezone", config.DEFAULT_TZ);
        }
