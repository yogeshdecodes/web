...
export async function linkProjects(makerlogProject, todoistProject) {
	try {
        const { data } = await axios.post('/apps/todoist/link/', { makerlog_project: makerlogProject, todoist_project: todoistProject });
		return data;
	} catch (e) {
		prettyAxiosError(e)
	}
...
export async function installApp(code) {
	try {
		const resp = await axios.post(`/apps/todoist/register/`, {code: code});
		return resp.success;
	} catch (e) {
		prettyAxiosError(e)
	}
