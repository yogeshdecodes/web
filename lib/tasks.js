...
async function getStreamMetadata(following=false) {
    try {
        const meta = await Promise.all(Object.keys(config.STREAM_TYPES()).map(
            type => fetchStreamType(type, following)
        ))
        return meta
    } catch (e) {
        console.log(e)
...
async function createTask(content, done=false, attachment=null, catchErr=true) {
    try {
...
async function syncTasks(lastSynced=null) {
    try {
      const url = lastSynced ? `/tasks/sync/?last_sync_date=${lastSynced}` : '/tasks/sync/';
      const response = await axios.get(url);
      return response.data
    } catch (e) {
        prettyAxiosError(e);
    ...
    try {
        ...
        if (tasksWithAttachments.length > 0) {
            let promises = tasksWithAttachments.map(
                async t => await createTask(t.content, t.done, t.attachment, false)
            )
...
async function postComment(id, content) {
    try {
        const response = await axios.post(`/tasks/${id}/comments/`, { content });
        return response.data
    } catch (e) {
        prettyAxiosError(e)
    }
...
export async function editComment(taskId, commentId, content) {
    try {
        const response = await axios.patch(`/tasks/${taskId}/comments/${commentId}/`, { content });
        return response.data
    } catch (e) {
        prettyAxiosError(e)
    }
