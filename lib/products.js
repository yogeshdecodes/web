...
export async function getProductPeople(slug, withCacheKey = false) {
    try {
        let url = `/products/${slug}/people/?cachekey=k43k3kjs`;
        if (withCacheKey) {
            url += `?cachekey=${Math.random()
                .toString(36)
                .substring(7)}`;
        }
...
export async function createProduct(
    ...
    try {
        ...
        if (accent) data.append("accent", accent);
...
export async function updateTeam(slug, team) {
    try {
        let data = new FormData();
        const headers = {
            "Content-Type": "multipart/form-data"
