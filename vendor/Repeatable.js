...
class Repeatable extends PureComponent {
    ...
    acquireTimer = () => {
        ...
        const repeatInterval = Math.max(
            Number(this.props.repeatInterval) || 0,
            0
        );
    ...
    render() {
        ...
        return (
            <Component
                role="presentation"
                {...props}
                onMouseLeave={this.releaseTimer}
                onMouseDown={chainedFunction(onMouseDown, event => {
                    if (disabled) {
                        return;
                    }
                    press(event);
                })}
                onTouchStart={chainedFunction(onTouchStart, event => {
                    if (disabled) {
                        return;
                    }
                    press(event);
                })}
                onTouchCancel={chainedFunction(onTouchCancel, event => {
                    if (disabled) {
                        return;
                    }
                    release(event);
                })}
                onTouchEnd={chainedFunction(onTouchEnd, event => {
                    if (disabled) {
                        return;
                    }
                    release(event);
