...
export const editorReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case types.ADD_TO_QUEUE:
            let task = {content: state.editorValue, done: state.editorDone, in_progress: state.editorInProgress, due_at: state.editorDueAt};
        ...
        case types.REMOVE_FROM_QUEUE:
            return {
                ...state,
                queue: [...state.queue].filter(t => t.content !== action.task.content),
            }
...
export const actions = {
    ...
    setEditorDueAt: (value) => ({ type: types.EDITOR_SET_DUE_AT, editorDueAt: value }),
    setEditorValue: (value) => ({ type: types.SET_EDITOR_VALUE, value: value }),
    setEditorAttachment: (attachment) => ({ type: types.SET_EDITOR_ATTACHMENT, attachment: attachment }),
    ...
    openDiscussionEditor: (toggle=true) => ({ type: toggle ? types.EDITOR_TOGGLE_DISCUSSIONS : types.EDITOR_OPEN_DISCUSSIONS }),
    ...
    createFailed: (errorMessages = ["Creation failed."], fieldErrors=null) => {
        return {
