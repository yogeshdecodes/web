...
export const tasksReducer = (state = initialState, action) => {
	switch (action.type) {
		...
		case types.TASKS_PURGE_BATCH:
			return {
				...state,
				tasks: state.tasks.filter(task => action.batch.indexOf(task.id) >= 0)
			}
		...
		case types.TASKS_REORDER:
			...
			const beforeIndex = findIndex(state.tasks, task => task.id === action.id);
			const afterIndex = findIndex(state.tasks, task => task.id === action.afterId);
...
export const actions = {
	...
	updateTaskSucceed: (task) => ({type: types.TASKS_UPDATE_SUCCEED, task: task,}),
	updateTaskFailed: () => ({type: types.TASKS_UPDATE_FAILED}),
	updateTask: (id, task = {}) => ({type: types.TASKS_UPDATE_REQUEST, id: id, payload: task}),
	reorderTask: (id, afterId) => ({type: types.TASKS_REORDER, id: id, afterId: afterId}),
