...
export const streamReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case types.STREAM_FETCH_SUCCEED:
            ...
            if (action.tasks) {
                newState.tasks = orderByDate(uniqBy([...action.tasks, ...state.tasks], 'id'))
            }
            if (action.milestones) {
                newState.milestones = uniqBy([...action.milestones, ...state.milestones], 'id')
            }
            return newState
        ...
        case types.STREAM_REMOVE_MILESTONE:
            const removedMilestones = state.milestones.filter(t => t.id !== action.id)
            return {
                ...state,
        ...
        case types.STREAM_MERGE_TASKS:
            ...
            if (action.tasks) {
                lastUpdatedTime = new Date(Math.max.apply(null, action.tasks.map(function(e) {
            ...
            return {
                ...
                tasks: orderByDate(uniqBy([...action.tasks, ...state.tasks], 'id')),
            }
        ...
        case types.STREAM_MERGE_MILESTONES:
            return {
                ...
                fetchFailed: false,
                allLoaded: false,
                milestones: orderByDate(uniqBy([...action.milestones, ...state.milestones], 'id')),
            }
...
export const actions = {
    ...
    mergeMilestones: (milestones) => ({ type: types.STREAM_MERGE_MILESTONES, milestones }),
