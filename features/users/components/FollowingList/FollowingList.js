...
const FollowingCard = ({hasFollows, children}) => (
	<div className={"card"}>
		...
		<div className={"card-content"} style={{padding: 20, paddingTop: 10, paddingBottom: 10, maxHeight: 400, overflowY: "auto"}}>
			{children}
		</div>
		{!hasFollows &&
    ...
    render() {
		...
		return (
			<FollowingCard hasFollows={!this.state.hasNoFollows}>
				...
				<div className={"center"}><h4>You aren't following anyone yet.</h4></div>
				}

				{this.state.people.map(
					(user) => <div><UserMedia user={user}/></div>
				)}
			</FollowingCard>
		)
	}
