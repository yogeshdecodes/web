...
class UserContainer extends React.Component {
    ...
    render() {
        ...
        if (this.state.loading || (!this.props.id && !this.props.username && !this.props.all)) {
            return <Spinner small={true} />;
        }

		if (this.state.failed && this.state.loading === false) {
			return <h3 className={"has-text-centered"}>Failed to load user. <button onClick={() => this.fetchUser()}>Try again.</button></h3>
