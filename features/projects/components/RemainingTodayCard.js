...
const HelpTooltip = props => (
    <Tooltip
        html={
            <span>
                <FontAwesomeIcon icon={"check"} /> <b>Click</b> to mark as done
            </span>
        }
        delay={200}
        position={"right"}
...
const RemainingTodayCard = props => {
    if (!props.showGreeting && (props.tasks.length === 0 || !props.tasks))
        return null;

    var d = new Date();
    const tasks = props.tasks.filter(
        task => (!task.done && wasAddedToday(task)) || isDueSoon(task)
    );

    return (
        <div
            className={
                "card RemainingTodayCard " +
                (props.showGreeting && tasks.length === 0 && "empty")
            }
        >
            <div className={"card-content"}>
                {tasks.length > 0 && (
                    <h3 className={"heading"}>Remaining today</h3>
                )}
