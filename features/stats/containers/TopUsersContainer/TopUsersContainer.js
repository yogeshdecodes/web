...
class TopUsersContainer extends React.Component {
	...
	fetchUsers = async () => {
		try {
			...
			this.setState({isLoading: false, ready: true, failed: false, users: stats.top_users})
		} catch (e) {
	...
	render() {
		if (this.state.ready && this.state.users && !this.state.failed) {
			...
			let propName = this.props.propName ? this.props.propName : 'topUsers';
			let props = {};
			props[propName] = this.state.users;
			return <Component {...props} />
		} else if (this.state.failed) {
			return <div>Failed to load top users. <button onClick={this.fetchUsers}>Retry</button></div>
		} else {
