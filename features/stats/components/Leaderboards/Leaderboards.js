        ...
        <div className={"Leaderboards-Row"} onClick={expand}>
            ...
            <div className={"Leaderboards-Content"}>
                <div className={"flex"}>
                    ...
                    <div>
                        <strong>
                            <FullName user={user} prependUsername={true} />
                        </strong>
                    </div>
                </div>
            </div>
...
class Leaderboards extends React.Component {
    getUserPosition = id => {
        const userIndex = this.props.topUsers.findIndex(user => user.id === id);
    ...
    renderUsers = () => {
        const users = this.props.topUsers.map((user, pos) => (
            <LeaderboardsRow
                user={user}
                rank={pos + 1}
                isCurrentUser={this.props.me && user.id === this.props.me.id}
            />
        ));
        ...
        if (
            ...
            this.props.me &&
            this.getUserPosition(this.props.me.id) === null
        ) {
            users.push(
                <LeaderboardsRow
                    user={this.props.me}
