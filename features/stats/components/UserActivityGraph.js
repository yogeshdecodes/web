...
class UserActivityGraph extends React.Component {
	...
	render() {
		...
		if (graph && (!graph.data || !graph.max || !graph.avg)) {
            return <div className={"center"}>This maker has no activity yet.</div>
		}

		if (failed) return <div>Failed to load. <button onClick={this.getActivityGraph}>Retry</button></div>
		...
		return (
			<CalendarHeatmap
				...
				values={graph.data.map(({date, count}) => ({date: new Date(date), count: count}))}
			/>
		);
	}
}
